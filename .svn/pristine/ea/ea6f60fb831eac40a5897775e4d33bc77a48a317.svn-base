...
//          }
	...
	public function authCode($string, $operation = 'DECODE', $key = '', $expiry = 0) {
	    ...
	    */
	    $ckey_length = 0;
	    $key = md5($key ? $key : md5(COOKIE_AUTHKEY.$_SERVER['HTTP_USER_AGENT']));
	...
	*/
	public function setCookie($var, $value, $life=0, $prefix = 1) {
	   	...
	   	//setcookie(($prefix ? $this->cookie_prefix : '').$var, $value, $life?time()+$life:0, $this->cookie_path, $this->cookie_domain, $_SERVER['SERVER_PORT'] == 443 ? 1 : 0);
                setcookie(($prefix ? COOKIE_PREFIX: '').$var, $value, $life?time()+$life:0, COOKIE_PATH, COOKIE_DOMAIN, $_SERVER['SERVER_PORT'] == 443 ? 1 : 0);
	}
