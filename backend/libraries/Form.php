...
class Form {
   ...
   // private $priv;
	...
	public static function images($name, $id = '', $value = '', $moudle='', $catid='', $size = 50, $class = '', $ext = '', $alowexts = '',$thumb_setting = array(),$watermark_setting = 0 ) {
		...
		//if(!defined('IMAGES_INIT')) {
			$str = '<script type="text/javascript" src="views/javascript/swfupload/swf2ckeditor.js"></script>';
			//define('IMAGES_INIT', 1);
		//}
		$authkey = upload_key("1,$alowexts,1,$thumb_ext,$watermark_setting");
		return $str."<input type=\"text\" name=\"$name\" id=\"$id\" value=\"$value\" size=\"$size\" class=\"$class\" $ext/>  <input type=\"button\" class=\"button\" onclick=\"javascript:flashupload('{$id}_images', '".$CI->lang->line('attachmentupload')."','{$id}',submit_images,'1,{$alowexts},1,{$thumb_ext},{$watermark_setting}','{$moudle}','{$catid}','{$authkey}')\" value=\"".$CI->lang->line('imagesupload')."\">";
	}
	 ...
	 */
	public static function select_category($file = '',$catid = 0, $str = '', $default_option = '', $modelid = 0, $type = -1, $onlysub = 0,$is_push = 0) {
		global $CI;
		if (!$file) {
		...
		$result = unserialize($CI->cache->get($file));
		...
		if($is_push=='1'){
			...
			$user_groupid = $this->input->cookie('_groupid') ? $this->input->cookie('_groupid') : 8;
		}
		if (is_array($result)) {
			foreach($result as $r) {
 				//检查当前会员组，在该栏目处是否允许投稿？
				if($is_push=='1' and $r['child']=='0'){
					$where = " where catid={$r['catid']} and roleid={$user_groupid} and action='add'";
					$array = $CI->model_category_priv->getCategory($where);
					if(!$array){
						continue;	
					}
				}
				...
				if($modelid && $r['modelid']!= $modelid ) unset($categories[$r['catid']]);
			}
		}
		$str  = "<option value='\$catid' \$selected>\$spacer \$catname</option>;";
		$str2 = "<optgroup label='\$spacer \$catname'></optgroup>";
    
		$CI->tree->init($categories);
		$string .= $CI->tree->getTreeCategory(0, $str, $str2);
	 ...
	 * @param $pre 模板前缀
	 */
	public static function selectTemplate($style, $module, $id = '', $str = '', $pre = '') {
        global $CI;
		$tpl_root = 'template';
		$templatedir = FRONTPATH.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR;
		$confing_path = FRONTPATH.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.'config.php';
		...
		ksort($templates);
		return self::select($templates, $id, $str,$CI->lang->line('please_select'));
	}
	...
	public static function urlrule($module, $file, $ishtml, $id, $str = '', $default_option = '') {
        ...
        $CI->load->driver('cache', array('adapter' => 'file', 'backup' => 'memcached'));
		$urlrules = unserialize($CI->cache->get('urlrules_detail'));
		$array = array();
