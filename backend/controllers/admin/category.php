...
class Category extends CI_Controller {
        ...
        if (!empty($result)) {
            foreach ($result as $r) {
                ...
                if ($r['url']) {
                    ...
                    $r['url'] = "<a href='?m=admin&c=category&a=public_cache&menuid=43&module=admin'><font color='red'>" . $this->lang->line('update_backup') . "</font></a>";
                }
    ...
    public function edit() {
        ...
        if ($this->input->post('dosubmit')) {
            ...
            $this->input->post('info')['sethtml'] = $setting['create_to_html_root'];
            $this->input->post('info')['setting'] = array2string($setting);
            $this->input->post('info')['module'] = 'content';
           ...
           // $_POST['info']['letter'] = strtolower(implode('', $letters));
            $this->input->post('info')['letter'] = $this->input->post('info')['catdir'];
            ...
            if ($this->input->post('template_child')) {
                ...
                $idstr = $this->getArrChildId($categories,$catid);
                if (!empty($idstr)) {
                    ...
                    if (!empty($arr)) {
                        foreach ($arr as $v) {
                            ...
                            $this->model_category->updateCategory( 'catid=' . $v['catid']," where setting ={$new_setting}");
                        }
                    }
                }
            }
            $sql=$this->db->update_string("{$this->db->dbprefix}category",$this->input->post('info'),array('catid' => $catid));
            $this->db->query($sql);
...
//		setcache('category_content',$array,'commons');
    ...
    public function publicTplFileList() {
        ...
        $batch_str = $this->input->get('batch_str');
        $batch_str = isset($batch_str) ? '[' . $catid . ']' : '';
        if ($catid) {
